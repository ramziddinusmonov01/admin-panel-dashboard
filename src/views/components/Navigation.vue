      ...
      <Modal :activeModal="activeModal" @open="changeActive" @close="closeModal" @submit="sendAdd" :isImage="true"
        ...
        <template v-slot:body>
          <div class="space-y-5 pb-5">
            <div class="space-y-3">
              <p>Menyu nomi</p>
              <input type="text" placeholder="" v-model="add_name"
          ...
          <table class="w-full table-auto">
            ...
            <tbody class="text-gray-600 text-sm font-light">
              <tr v-for="(item, index) in items?.data" :key="item.id"
                ...
                <td class="py-4 px-6 text-left">
                  <div class="flex-1">
                     <span v-if="item.status === 1" class="bg-green-200 text-green-700 py-1 px-4 rounded-full text-xs">Faol</span>
                  ...
                  <div class="flex item-center justify-center">
                    ...
                    <Modal :activeModal="activeModal" @click="openEdit(item)" @open="changeActive" @close="closeModal"
                      ...
                      <template v-slot:body>
                        <div>
                          <div class="flex items-center">
                            <span class="font-medium text-lg flex-1">Menyu nomi</span>
                      ...
                      <template v-slot:button>
                        <div class="w-8 h-8 flex items-center justify-center border-blue-500 rounded mr-2 transform hover:text-blue-500 hover:scale-110 border-2 " >
                          <svg class="w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" fill="white"/>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
...
function openEdit(item) {
  edit_name.value = item.category_name;
...
async function sendEdit(item) {
  let id = item.id;
  let formdata = new FormData();
  formdata.append("category_name", edit_name.value);
...
async function sendDelete(item) {
  ...
  await submitDelete(adminUrl + "/navbar-category/delete?id=" + id);
  await getData();
}
